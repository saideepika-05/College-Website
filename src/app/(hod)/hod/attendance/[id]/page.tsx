...
import {
  hodCloseAttendanceSession,
  hodEditAttendanceRecord,
...
export default async function HodSessionPage({
  params,
}: {
  ...
  if (!departmentIds.includes(session.departmentId)) notFound();
  ...
  if (isLive) {
    return (
      <LiveSession
        ...
        closeAction={hodCloseAttendanceSession}
      />
    );
  }
  ...
  return (
    <>
      ...
      <RecordsEditor records={records} editAction={hodEditAttendanceRecord} />
    </>
  );
}
